...
if [[ "$URL" =~ episode=([0-9]+) ]]; then
    ...
    http GET "$SCHEDULE" referer:https://animego.me X-Requested-With:XMLHttpRequest \
        ...
        | htmlq \
        | xq --arg url "$URL" '.html.body.div | to_entries | group_by(.key / 4 | trunc) | map(.[2].value + .[3].value | select(.["#text"]? | not) | {
                url: (.["@data-number"] as $num | "\($url)&episode=\($num)"),
                title: .["@data-number"]
            }) | {
                list: .,
                hashkey: "url",
